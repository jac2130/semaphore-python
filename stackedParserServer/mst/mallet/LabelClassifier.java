...
import mst.FeatureVector;
import mst.Parameters;
...
public class LabelClassifier
{
	...
	public void passThroughInstancesToSelectFeatureThresholds() throws Exception
	{
		...
		int j = 0;
		System.out.println("Passing through instances to select feature thresholds...");
		System.out.print("  [");
	    ...
	    System.out.println("No of active features:"+predCounts.size());
	}
	...
	{	
		...
		int numInstances = instanceLengths.length;
		int j = 0;
		System.out.println("Storing feature vectors in Mallet data structure...");
		System.out.print("  [");
		long startTime = (new Date()).getTime();
		for(int i = 0; i < numInstances; i++) {
			if((i+1) % 500 == 0) {
	    ...
	    long endTime = (new Date()).getTime();
	    System.out.println(" |Time:"+(endTime-startTime)+"]");
	    System.out.println("Number of labels: " + tAlph.size());
	    System.out.println("Number of Predicates: " + dataAlph.size());
	    ...
	    System.out.println("Done Training Labeler\nReturning classifier");
	    System.setErr(err);
		return classifier;
	}	
